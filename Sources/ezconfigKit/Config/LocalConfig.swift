...
enum SetupError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case let .notInitialized(path):
            return """
            Could not find \(path).
            This project has not been initialised yet. Whoever set up the repo
            needs to run:
              ezconfig init
        ...
        case let .noBundlePrefix(path):
            return """
            \(path) exists but has no BUNDLE_PREFIX line.
            It was probably edited by hand, or it was not generated by ezconfig.
            """
        case let .unusableTeamID(id):
            return "Team ID '\(id)' cannot be turned into a bundle ID suffix."
        }
    }
}
